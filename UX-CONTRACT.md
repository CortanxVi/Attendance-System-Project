...
| Hybrid face check-in | signed protocol-v4 challenge bound to account/session selects one blink or short move closer after three passive frames; no card after QR | continuous camera progress, one plain-language action, then disabled submit + spinner | result card | result + realtime toast | retry the 20-second camera attempt within the one-use 7-minute QR authorization; rescan QR only on expiry | live camera instruction then result heading/live status | `LivenessScanner.tsx`, `StudentHome.tsx`, `liveness_service.py`, `liveness_frame_service.py` |
...
- Liveness validation: face enrollment retains signed protocol v3 passive mode. Attendance uses signed protocol v4 hybrid mode: three passive samples followed by one randomly selected low-friction action (`blink` or `move_closer`) after a signed 500–1,400 ms prompt delay, then a recovery frame. The token binds challenge, authenticated account, attendance session, expiry, nonce, action, and delay. Browser Face Landmarker runs in a Web Worker and captures the exact analyzed frames, but its decision is advisory. Backend recomputes one-face presence, same-person similarity, passive MiniFAS PAD, action geometry, recovery, timing order, and frozen-frame resistance before face recognition. Supabase claim/finalize RPCs retain one-use and concurrent-request protection. Static duplicates, wrong actions, face loss/swap, expired/borrowed tokens, print/screen risk, and repeated finalization fail closed. This raises the cost of ordinary replay but does not claim resistance to adaptive deepfake/virtual-camera injection or ISO/IEC 30107 certification.
...
- Student face self-enrollment: OCR must first bind the card number to the authenticated profile, with separate camera and file-picker choices, then a short-lived signed and account-bound challenge drives protocol-v3 passive liveness. The backend recomputes person continuity, frame continuity, and passive PAD before atomically storing the final verified embedding and consuming the challenge. A student cannot overwrite an existing embedding; only a permanent administrator may use the supervised override flow.
...
- Attendance scope: check-in only; passive liveness frames are transient request data and are not retained as permanent attendance photos. Student-card images are used only for initial self-enrollment OCR, never after Dynamic QR check-in.
