...
| Face + OCR check-in | signed blink + random turn, then submit face/turn/card | progress then disabled + spinner | result card | result + realtime toast | retain card; retry without rescan on transient OCR/database failure, rescan only on expiry | result heading/live status | `LivenessScanner.tsx`, `StudentHome.tsx` |
...
- Liveness validation: browser requires two-eye blink and randomized left/right turn with calibrated yaw/pitch; backend verifies challenge signature/timing, one face in closed-eye/turned/frontal frames, eyelid aperture change, direction geometry and same-person similarity. A nod, one-eye wink, or face-loss/swap resets or fails the flow. This is challenge-response hardening, not certified presentation-attack detection.
