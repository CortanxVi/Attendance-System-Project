...
- Liveness validation: signed protocol v3 declares passive mode and exactly three samples. Browser Face Landmarker runs in a Web Worker, requires one centered face, accepts at least 8 effective FPS, and captures three analyzed frames at bounded intervals without asking the user to move or blink. A camera attempt lasts at most 20 seconds and can be restarted within the account-bound, one-use 7-minute authorization. Backend treats browser timing as untrusted and recomputes one-face presence, same-person similarity, frontal continuity, frozen-frame resistance, and a local three-frame MiniFAS passive PAD gate before face recognition. A static duplicate, face loss/swap, print or screen risk fails closed. This is RGB-camera PAD hardening and is not an ISO/IEC 30107 certification claim.
