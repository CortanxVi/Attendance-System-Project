...
export interface ReportCourse { id: string; course_code: string; course_name: string; semester: number; year: number; section: number; profiles?: { full_name?: string | null } | null }
...
export async function exportAttendanceReport(records: AttendanceExportRecord[], courseCode: string, format: ReportFormat, accentColor: string, instructorName?: string | null) {
  ...
  const rows = records.map((row, index) => ({
    ...
    ผู้สอน: instructorName || 'ไม่ระบุ', สถานะ: statusLabel(row.status), วิธีการ: methodLabel(row.method), เวลา: new Date(row.check_in_time).toLocaleString('th-TH'),
  }));
  if (format !== 'pdf') {
  ...
  const documentDefinition: TDocumentDefinitions = {
    ...
    content: [
      { text: `รายงานประวัติการเข้าเรียน - รหัสวิชา: ${courseCode}`, style: 'header', margin: [0, 0, 0, 4] },
      { text: `ผู้สอน: ${instructorName || 'ไม่ระบุ'}`, margin: [0, 0, 0, 15] },
