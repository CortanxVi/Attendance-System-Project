...
export default function TeacherDashboard() {
  ...
  const handleStartSession = async (course: Course) => {
    try {
      await attendance.startSession(course);
      notify(`เปิดระบบเช็คชื่อวิชา ${course.course_code} แล้ว`, 'success');
    } catch (error: unknown) {
  ...
  const confirmAction = async () => {
    ...
    if (pendingAction.type === 'close-session') {
      ...
      try {
        ...
        notify(apiDetail(error, 'เกิดข้อผิดพลาดในการเชื่อมต่อเพื่อปิดเซสชัน'), 'error');
      }
    }
  };
  ...
  return (
    <div className="flex min-w-0 flex-col bg-transparent">
      <ConfirmDialog
        ...
        busy={attendance.mutating}
        onConfirm={confirmAction}
        onCancel={() => setPendingAction(null)}
      />
      
      {/* โซน Modals ต่างๆ (โค้ดเดิม) */}
      {attendance.activePanel === 'qr' && attendance.activeSession && <LiveAttendance courseCode={attendance.activeSession.course_code} courseName={attendance.activeSession.course_name} activeSessionId={attendance.activeSession.id} onClose={() => attendance.setActivePanel(null)} />}
      {attendance.activePanel === 'nfc' && attendance.activeSession && <NFCManager defaultCourseCode={attendance.activeSession.course_code} activeSessionId={attendance.activeSession.id} onClose={() => attendance.setActivePanel(null)} />}
      ...
      <div className="min-w-0 animate-fade-in">
        ...
        {attendance.recoveryError && (
          <div role="alert" className="mb-5 flex flex-col gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 sm:flex-row sm:items-center sm:justify-between">
            <span>{attendance.recoveryError}</span>
            <button type="button" onClick={() => void attendance.refreshActiveSession()} className="min-h-10 cursor-pointer rounded-xl border border-amber-300 bg-white px-4 font-bold hover:bg-amber-100 focus:outline-none focus:ring-2 focus:ring-amber-400">ตรวจสอบอีกครั้ง</button>
          </div>
        )}
        ...
        {attendance.activeSession && (
          <LiveCheckInFeed sessionId={attendance.activeSession.id} courseCode={attendance.activeSession.course_code} />
        )}
        ...
        {isLoading ? (
          ...
          <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "flex flex-col gap-4"}>
            {courses.map((course) => {
              const isThisCourseActive = attendance.activeSession?.course_id === course.id;
              return (
                <div key={course.id} className={`flex min-w-0 rounded-2xl border border-gray-200 bg-white shadow-sm transition-all hover:shadow-md ${viewMode === 'list' ? 'flex-col gap-4 p-4 md:flex-row md:items-center' : 'flex-col p-4 sm:p-6'}`}>
                  ...
                  <div className={`${viewMode === 'list' ? 'w-full border-t border-gray-100 pt-4 md:w-[clamp(13rem,25%,16rem)] md:shrink-0 md:border-t-0 md:border-l md:pt-0 md:pl-4' : 'mt-6'}`}>
                    {!attendance.activeSession ? (
                      <button onClick={() => void handleStartSession(course)} disabled={attendance.loading || attendance.mutating} className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-slate-800 py-3 text-sm font-medium text-white shadow-sm transition-colors hover:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-300 disabled:cursor-not-allowed disabled:opacity-50">
                        <Lucide.Power size={18} /> {attendance.loading ? 'กำลังตรวจสอบเซสชัน...' : attendance.mutating ? 'กำลังเปิดระบบ...' : 'เปิดระบบเช็คชื่อ'}
                      </button>
                    ) : isThisCourseActive ? (
                      <div className="space-y-2 bg-emerald-50/50 p-3 rounded-xl border border-emerald-100">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <button onClick={() => attendance.setActivePanel('qr')} className="flex cursor-pointer items-center justify-center gap-1 rounded-lg border border-gray-300 bg-white px-2 py-1.5 text-xs font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-orange-300"><Lucide.QrCode size={14} /> จอ QR</button>
                          <button onClick={() => attendance.setActivePanel('nfc')} className="flex cursor-pointer items-center justify-center gap-1 rounded-lg bg-slate-800 px-2 py-1.5 text-xs font-medium text-white shadow-sm hover:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-300"><Lucide.CreditCard size={14} /> NFC</button>
                        </div>
                        <button onClick={handleEndSession} disabled={attendance.mutating} className="mt-1 w-full cursor-pointer rounded-lg border border-red-200 bg-white py-1.5 text-center text-xs font-semibold text-red-600 hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-300 disabled:cursor-not-allowed disabled:opacity-50">ปิดระบบ</button>
                      </div>
                    ) : (
