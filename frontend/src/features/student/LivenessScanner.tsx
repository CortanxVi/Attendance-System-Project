...
export default function LivenessScanner({ onCaptureSuccess }: { onCaptureSuccess: (capture: LivenessCapture) => void }) {
  ...
  useEffect(() => {
    ...
    const markers: PassiveLivenessEvidence['frames'] = [];
    const observedTimestamps: number[] = [];

    const fail = (message: string) => {
      if (cancelled) return;
      setIsLoading(false); setError(message);
    };
    const processFrame = async (timestamp: number) => {
      if (cancelled) return;
      const video = webcamRef.current?.video;
      if (workerReady && video && video.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA && !processing && timestamp - lastProcessedAt >= 50 && samples.length < SAMPLE_COUNT) {
        processing = true; lastProcessedAt = timestamp;
    ...
    const handleMessage = (message: FaceLandmarkerClientEvent) => {
      ...
      try {
        ...
        stableFrames += 1;
        setIsLoading(false);
        setInstruction('มองตรงนิ่ง ๆ ระบบกำลังตรวจว่าเป็นบุคคลจริง');
        if (stableFrames >= 4 && (samples.length === 0 || observation.timestampMs - lastSampleAt >= SAMPLE_INTERVAL_MS)) {
          ...
          setProgress(samples.length / SAMPLE_COUNT);
          if (samples.length === SAMPLE_COUNT) {
            ...
            callbackRef.current({
              ...
              evidence: { version: 3, mode: 'passive', sampleCount: SAMPLE_COUNT, frames: markers, startedAtMs: startedAt, completedAtMs: observation.timestampMs, effectiveFps },
            });
          }
        }
    ...
    void openFaceLandmarkerClient(handleMessage).then((nextClient) => {
      ...
      client = nextClient; workerReady = true; setInstruction('วางใบหน้าให้อยู่กลางกรอบ'); animationFrame = requestAnimationFrame(processFrame);
  ...
  }, [attemptKey, selectedCameraId]);

  return <div data-disable-pull-refresh className="relative flex min-h-[clamp(20rem,64dvh,27rem)] w-full flex-col items-center justify-center overflow-hidden rounded-2xl bg-gray-950">
