...
หน้า Teacher/Admin ใช้พื้นที่แบบ fluid เต็มความกว้างโดยไม่กำหนดความกว้างหน้าแบบตายตัว; sidebar เป็น drawer บนมือถือ/แท็บเล็ตและคงอยู่เมื่อ viewport กว้างตั้งแต่ 1024px โดยผู้ใช้กดยุบ/ขยายได้ ระบบจำค่าบนอุปกรณ์ และเมื่อยุบให้ขยายชั่วคราวเมื่อชี้เมาส์. พื้นที่เนื้อหาต้องมี `min-width: 0` และตารางเป็นเจ้าของ horizontal scroll ของตนเอง. Padding เปลี่ยนตาม viewport ตั้งแต่ 0.75rem ถึง 2rem และใช้จังหวะ 1.5rem ระหว่าง section. ทุก role มีปุ่มย้อนกลับภายใน shell. ระบบไม่ดัก gesture เพื่อทำ Pull to Refresh และไม่สั่ง reload ทั้งหน้าโดยอัตโนมัติ; การโหลดข้อมูลใหม่เป็น action ของแต่ละหน้าหรือกลไก browser ตามปกติ.

Student เป็น mobile-only shell ขนาดกว้าง 100% และจำกัดเพดาน 480px เพื่อครอบคลุมโทรศัพท์หน้าจอแคบ 320px, iPhone/Android รุ่นทั่วไป และจอใหญ่โดยยังคงสัดส่วนกลางที่อ่านง่าย. Shell สูงตาม `100dvh` (fallback `100svh`), รองรับ iOS safe-area และมี vertical scroll owner เพียงจุดเดียวที่ main content; bottom navigation อยู่นอก scroller และต้องมองเห็นอยู่เสมอ. ระยะขอบหน้าใช้ `clamp(0.75rem, 4vw, 1.25rem)` ผ่านคลาสกลาง `student-page`; breakpoint ของ viewport ภายนอกห้ามทำให้คอนเทนต์นักศึกษาเปลี่ยนเป็น desktop layout ภายใน shell. Upload และ action สำคัญต้องเข้าถึงได้โดยไม่ล็อกความสูงหน้าฟอร์ม
...
หน้าตรวจ liveness ใช้กล้อง stream เดียว แสดง native camera selector เมื่อมีกล้องหลายตัว และมี progress/instruction ที่อ่านได้โดย screen reader. กล้องหน้าต้องแสดง preview แบบ mirror ส่วนเฟรมตรวจสอบคงข้อมูลกล้องจริง. หน้าเริ่มเช็คชื่อต้อง preload, compile และ warm-up Face Landmarker แบบ singleton ก่อนเปิดปุ่มสแกน QR; loading ต้องมีพื้นที่คงที่ ส่วน failure แสดงปุ่มลองใหม่และห้ามเปิด QR จนกว่าโมเดลพร้อม. หลัง QR ล่าสุดผ่าน server ออกสิทธิ์แบบผูกบัญชีและใช้ครั้งเดียวซึ่งลองสแกนซ้ำได้รวม 7 นาที แต่กล้องแต่ละรอบต้องจบภายใน 20 วินาที. การลงทะเบียนใบหน้าคง Protocol v3 แบบ Passive 3 เฟรม ส่วนการเช็กชื่อบน branch ทดลองใช้ Protocol v4: เก็บ Passive 3 เฟรม แล้วแสดงคำสั่งสุ่มง่าย ๆ เพียงหนึ่งครั้งให้กะพริบตาหรือขยับเข้าใกล้ ก่อนเก็บภาพกลับสู่ท่าปกติ. Backend ตรวจใบหน้าเดียว บุคคลเดียวกัน ความต่อเนื่องของภาพ รูปทรงคำสั่งที่ลงนาม และ MiniFAS passive PAD แบบ local ก่อนเปรียบเทียบใบหน้า แล้วทิ้งภาพโดยไม่บันทึกถาวร. หลัง Dynamic QR ห้ามขอภาพบัตรซ้ำ; ใช้บัญชีที่ยืนยันแล้ว challenge ที่ผูกผู้ใช้/session และใบหน้าที่ลงทะเบียนเป็นขอบเขตยืนยันตัวตน. สีเขียวใช้เมื่อกำลังยืนยันและสำเร็จเท่านั้น ส่วน error แสดงสาเหตุพร้อมทางเริ่มใหม่

กล้องสแกน QR ของนักศึกษาเป็น full-viewport takeover หลังผู้ใช้กดเปิดกล้องเท่านั้น ใช้กล้องหลังเป็นค่าเริ่มต้น วิดีโอเต็มจอมีกรอบสแกนสีส้ม คำแนะนำด้านล่าง และปุ่มกลับที่เคารพ safe-area. สถานะ verifying/success/error ใช้แผงกลางที่คงตำแหน่ง; error ของสิทธิ์กล้องและ QR ไม่ถูกต้องต้องบอกวิธีแก้พร้อมปุ่มลองใหม่. Overlay ล็อก scroll, กัก focus, ปิดด้วย Escape/ปุ่มกลับ และคืน focus ให้ปุ่มเปิดกล้อง. เส้นสแกนหยุดนิ่งเมื่อเปิด reduced motion. รูปแบบอ้างอิงความคุ้นเคยจากเครื่องสแกน QR ของแอปธนาคาร แต่ไม่ลอก branding หรือสีของธนาคารใด
...
โปรไฟล์นักศึกษาใช้โครง mobile card: รูปวงกลมจาก Google Account พร้อม initials fallback, ชื่อ/อีเมล/รหัส, ชั้นปีแบบอ่านอย่างเดียวที่ backend คำนวณจากปีรับเข้าในรหัสนักศึกษา, ปุ่มออกจากระบบ และ action ลงทะเบียนใบหน้าเฉพาะสถานะที่ยังไม่ลงทะเบียน. คำร้องเป็น navigation row ไปยังหน้ารายการและ thread แยกที่มีทางกลับสู่โปรไฟล์; นักศึกษาและอาจารย์เห็นเฉพาะคู่สนทนาของรายวิชา. ไฟล์แนบใช้ shared preview card, แสดงตัวอย่างก่อนส่งและโหลดตัวอย่างที่ได้รับเมื่อผู้ใช้กดเท่านั้น

การลงทะเบียนใบหน้าด้วยตนเองต้อง preload โมเดลก่อน เปิดด้วยการตรวจบัตรที่เลือกได้ชัดเจนระหว่าง “ถ่ายรูป” และ “เลือกรูปจากเครื่อง” แล้วรับ challenge อายุสั้นแบบผูกบัญชีและใช้ครั้งเดียวเพื่อทำ Passive Liveness protocol v3. Backend เป็นผู้ตรวจ 3 เฟรมต่อเนื่องและใช้ embedding จากเฟรมสุดท้ายที่ผ่านการตรวจเท่านั้น; ห้ามรับภาพนิ่งเดี่ยวจากนักศึกษาและห้ามให้นักศึกษาเขียนทับใบหน้าที่ลงทะเบียนแล้ว. การแก้ไขข้อมูลเดิมเป็นงานควบคุมโดยผู้ดูแลระบบถาวร
...
หน้าจอ Dynamic QR และรหัสคลาสมี presentation mode ร่วมกัน: ปุ่มเต็มหน้าจอต้องมองเห็นได้ทั้ง touch/keyboard, ใช้ Fullscreen API เมื่อ browser รองรับ และใช้ app-owned viewport fallback เมื่อไม่รองรับ. โหมดนี้คงข้อมูลหลักไว้กึ่งกลาง อ่านได้จากหน้าห้อง มีปุ่มออกจากเต็มหน้าจอชัดเจน และไม่ใช้การขยายเพื่อเปลี่ยนสถานะคาบเรียน. ชั้น presentation อยู่เหนือ dialog ทั่วไปแต่ต่ำกว่า toast ผ่าน `--z-presentation` ใน `frontend/src/index.css`.

คาบเรียนที่เปิดอยู่เป็นสถานะจาก Backend และแสดงตัวบ่งชี้แบบ persistent ใน Teacher shell แม้เปลี่ยนหน้า. การปิดหน้าต่าง QR/NFC เป็นเพียงการย่อเครื่องมือ; การปิดคาบต้องใช้ action “ปิดระบบ” พร้อม confirmation โดยตรงเท่านั้น.
